...
export async function getDB() {
  ...
  // Always remove old challenges if they exist
  try {
  ...
  try {
    const weakPassword = await dbInstance.get('challenges', 'weakPassword')
    if (weakPassword) {
      await dbInstance.delete('challenges', 'weakPassword')
    }
  } catch (e) {
    // Challenge doesn't exist, that's fine
  }
  ...
  try {
    const reflectedXss = await dbInstance.get('challenges', 'reflectedXss')
    if (reflectedXss) {
      await dbInstance.delete('challenges', 'reflectedXss')
    }
  } catch (e) {
    // Challenge doesn't exist, that's fine
  }
...
async function seedDatabase(db: IDBPDatabase<JuiceShopDBSchema>) {
  ...
  // Users - Default credentials user (admin@juice-shop.com / admin123)
  await db.put('users', {
    id: 1,
  ...
  // Products (including VIP-only items)
  const products = [
  ...
  const challenges = [
    ...
    { key: 'sqlInjectionSearch', name: 'Reveal VIP Products', category: 'Injection', difficulty: 2, description: 'Use SQL injection to reveal hidden VIP products', solved: false },
    { key: 'storedXss', name: 'Stored XSS', category: 'Injection', difficulty: 2, description: 'Inject malicious code into product data', solved: false },
    
    // A05:2021 – Security Misconfiguration (Risk #5)
    { key: 'defaultCredentials', name: 'Use Default Credentials', category: 'Security Misconfiguration', difficulty: 1, description: 'Disarm alarm with default IoT credentials', solved: false },
    
    // A07:2021 – Identification and Authentication Failures (Risk #7)
    { key: 'loginAdmin', name: 'Login Admin', category: 'Authentication Failures', difficulty: 1, description: 'Log in with the admin account', solved: false },
...
export async function searchProducts(query: string): Promise<any[]> {
  ...
  return products.filter(p => {
    ...
    // If query contains SQL operators, bypass VIP check and show all products!
    if (query.includes(' OR ') || query.includes("' OR") || query.includes('1=1')) {
      return true // SQL injection - bypasses vipOnly filter!
    }
    
    // Normal behavior: Hide VIP products from regular search
    if (p.vipOnly) {
      return false // VIP products are hidden
    }
    
    // Simulating SQL LIKE query for regular products
...
export async function resetDatabase() {
  ...
  // Reseed challenges with the correct list (9 total)
  const challenges = [
    { key: 'accessAdminSection', name: 'Access Admin Section', category: 'Broken Access Control', difficulty: 1, description: 'Access the administration section without authorization', solved: false },
    { key: 'exposeSensitiveData', name: 'View Other Users\' Data', category: 'Broken Access Control', difficulty: 2, description: 'Access sensitive user data by entering database credentials', solved: false },
    { key: 'plaintextPassword', name: 'View Plaintext Passwords', category: 'Cryptographic Failures', difficulty: 2, description: 'Find a password stored in plain text', solved: false },
  ...
  // Reseed products (including VIP-only items)
  const products = [
    { id: 1, name: 'Orange Juice', description: 'Fresh orange juice', price: 2.99, vipOnly: false },
    { id: 2, name: 'Apple Juice', description: 'Crisp apple juice', price: 3.49, vipOnly: false },
    { id: 3, name: 'Mango Juice', description: 'Tropical mango juice', price: 4.99, vipOnly: false },
...
export async function getAllProducts() {
  const db = await getDB()
  const allProducts = await db.getAll('products')
  // Hide VIP products from regular display
  return allProducts.filter(p => !p.vipOnly)
}
...
export async function getUserScore(): Promise<number> {
  ...
  return completed.reduce((sum, ccKey) => {
    const challenge = challengeMap.get(ccKey)
