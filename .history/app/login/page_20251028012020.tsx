  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (email === 'admin@juice-shop.com') {
          const completed = await completeChallenge(userId, 'loginAdmin')
          if (completed && window.showChallengeToast) {
            window.showChallengeToast('Login as Admin Challenge Completed!')
          }
        }
        
        // Check for default credentials
        if (email === 'admin@juice-shop.com' && password === 'admin123') {
          const completed = await completeChallenge(userId, 'defaultCredentials')
          if (completed && window.showChallengeToast) {
            window.showChallengeToast('Use Default Credentials Challenge Completed!')
          }
        }
        
        // Check for weak password
        if (password === 'admin123') {
          const completed = await completeChallenge(userId, 'weakPassword')
          if (completed && window.showChallengeToast) {
            window.showChallengeToast('Use Weak Password Challenge Completed!')
          }
        }
