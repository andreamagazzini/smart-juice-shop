    ...
    try {
      ...
      if (data.success) {
        ...
        if (email === 'admin@juice-shop.com') {
          await completeChallenge(userId, 'loginAdmin')
        }
        
        // Check for default credentials
        if (email === 'admin@juice-shop.com' && password === 'admin123') {
          await completeChallenge(userId, 'defaultCredentials')
        }
        
        // Check for weak password
        if (password === 'admin123') {
          await completeChallenge(userId, 'weakPassword')
        }
        
        // Check for SQL injection in email
        if (email.includes("' OR") || email.includes(' OR ') || email.includes('=TRUE') || email.includes('#') || email.includes('--')) {
          await completeChallenge(userId, 'sqlInjectionLogin')
        }
