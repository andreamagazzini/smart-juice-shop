...
export default function CheckoutPage() {
  ...
  const handleCheckout = async () => {
    ...
    // Check for price manipulation (negative, zero, or suspiciously low prices)
    const hasSuspiciousPrice = cart.some(item => 
      item.price <= 0 || // Zero or negative
      (item.originalPrice && item.price < item.originalPrice) || // Lower than original
      item.price < 0.50 // Suspiciously cheap (juice can't cost less than 50 cents!)
    )
    
    if (hasSuspiciousPrice) {
      const completed = await completeChallenge('negativeOrder')
      if (completed && window.showChallengeToast) {
        window.showChallengeToast(
          'Manipulate Price',
          'You changed the prices in your browser and got items for cheaper (or free)! Websites should always recalculate prices on their servers, not trust what your browser says. It\'s like changing price tags at a store.'
        )
      }
    }
          ...
          {cart.length === 0 ? (
            ...
            <>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="w-16 px-2 py-2 border border-gray-300 rounded-lg text-gray-900 text-center"
                          min="1"
                        />
                      </div>
              ...
              <p className="mt-4 text-sm text-gray-500 text-center">
                💡 Hint: Prices can be manipulated in browser storage!
              </p>
            </>
          )}
        </div>
