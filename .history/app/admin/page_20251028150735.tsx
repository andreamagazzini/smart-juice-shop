  ...
  useEffect(() => {
    ...
    const markChallenge = async () => {
      try {
        ...
        const completed = await completeChallenge(userId, 'accessAdminSection')
        if (completed && window.showChallengeToast) {
          window.showChallengeToast('Access Admin Section Challenge Completed!')
        }
    ...
    if (newVisible.has(userId)) {
      ...
      if (userRole === 'admin') {
        try {
          ...
          await completeChallenge(userId, 'plaintextPassword')
          if (window.showChallengeToast) {
