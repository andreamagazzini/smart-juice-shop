...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const markChallenge = async () => {
      try {
        ...
        console.log('✅ Admin section access complete!')
      } catch (error) {
  ...
  // Mark plaintext password challenge when viewing Users tab with credentials
  useEffect(() => {
    async function markPlaintextPassword() {
      if (activeTab === 'Users' && hasCredentials) {
        try {
          await completeChallenge(1, 'plaintextPassword')
        } catch (error) {
          console.error('Error completing challenge:', error)
        }
      }
    }
    markPlaintextPassword()
  }, [activeTab, hasCredentials])
