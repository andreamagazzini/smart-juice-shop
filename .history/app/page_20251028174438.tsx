...
export default function Home() {
  ...
  const handleSearch = async (query: string) => {
    ...
    // Check for XSS payloads and execute them (simulating vulnerable behavior)
    if (query.includes('<script>') || query.includes('<img') || query.includes('onerror=')) {
      ...
      // Execute script tags by using eval (VERY VULNERABLE - for educational purposes only!)
      if (query.includes('<script>')) {
        const scriptContent = query.match(/<script>(.*?)<\/script>/)?.[1]
        if (scriptContent) {
          try {
            // eslint-disable-next-line no-eval
            eval(scriptContent)
          } catch (e) {
            console.error('XSS execution error:', e)
          }
        }
      }
    }
