    ...
    if (query.includes('<script>') || query.includes('<img') || query.includes('onerror=')) {
      ...
      if (completed && window.showChallengeToast) {
        window.showChallengeToast(
          'Reflected XSS',
          'You executed a Reflected Cross-Site Scripting attack! User input was rendered without sanitization. Always sanitize and escape user input, use Content Security Policy (CSP) headers, and validate input on both client and server sides.'
        )
      }
    }
    ...
    if (query.includes("' OR") || query.includes(' OR ') || query.includes('1=1') || query.includes('UNION') || query.includes('#')) {
      ...
      if (completed && window.showChallengeToast) {
        window.showChallengeToast(
          ...
          'You performed a SQL Injection attack! Direct string concatenation in queries allows attackers to manipulate database operations. Always use parameterized queries (prepared statements) and never trust user input. ORMs can help, but understand the underlying SQL.'
        )
      }
    }
