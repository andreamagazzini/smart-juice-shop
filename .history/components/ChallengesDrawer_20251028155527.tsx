...
export default function ChallengesDrawer() {
  ...
  const getChallengeHints = (key: string) => {
    const hints: Record<string, string[]> = {
      'loginAdmin': ['Log in with the admin account: admin@juice-shop.com / admin123'],
      'sessionHijacking': [
        'Copy a session token from the admin panel Users table',
        'Replace your session cookie in DevTools with that token',
        'Refresh the page - you should be logged in as that user!'
      ],
      'weakPassword': ['Log in using a weak password like "admin123"'],
      'plaintextPassword': ['Check the admin page Users tab', 'The admin password "admin123" is shown in plain text', 'This violates cryptographic best practices'],
      'defaultCredentials': [
      ...
      'reflectedXss': [
        'Try entering <script>alert(1)</script> in the search bar',
        'Or try <img src=x onerror=alert(1)>',
        'The search results will reflect your input without sanitization'
      ],
      'accessAdminSection': [
  ...
  return (
    <>
      ...
      {isOpen && (
        ...
        >
          {/* Left side - semi-transparent overlay (clickable to close) */}
          <div className="flex-1 cursor-pointer backdrop-blur-[2px] bg-black bg-opacity-[0.03]"></div>
          
          {/* Right side - Drawer panel */}
          <div 
            className="h-full w-[800px] bg-gradient-to-br from-purple-50 to-pink-100 overflow-y-auto shadow-2xl cursor-default"
