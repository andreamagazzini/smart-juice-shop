...
export default function ChallengesDrawer() {
  ...
  const getChallengeHints = (key: string) => {
    const hints: Record<string, string[]> = {
      ...
      'sqlInjectionSearch': [
        'Only 5 products show up normally',
        'Try searching for: 1=1'
      ],
      'storedXss': [
        'Go to Admin panel → Products tab',
        'Edit a product description',
        'Try: <img src=x onerror=alert(1)>',
        'The alert will show on the home page'
      ],
      ...
      'negativeOrder': [
        ...
        'Go to Local Storage → Find "cart"',
        'Change the "price" values to 0',
              ...
              {showImport && (
                <div className="mt-4">
                  <input
                    ...
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm text-gray-900 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-700"
                  />
                </div>
              )}
            </div>
