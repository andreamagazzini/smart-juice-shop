...
export default function Navigation() {
  ...
  const [toast, setToast] = useState({ show: false, message: '', explanation: '' })

  useEffect(() => {
    // Setup global toast function
    window.showChallengeToast = (message: string, explanation?: string) => {
      setToast({ show: true, message, explanation: explanation || '' })
    }
    ...
    const checkAuth = async () => {
      try {
        ...
        if (newUser) {
          ...
          const isAdminEmail = newUser.email === 'admin@juice-shop.com'
          ...
          if (previousUser && previousUser.email !== completeUser.email) {
            ...
            if (completed) {
              setToast({ 
                show: true, 
                message: 'Session Hijacking', 
                explanation: 'You successfully hijacked another user\'s session by copying their session token. This vulnerability occurs when session tokens are exposed (e.g., in admin panels) and not properly protected. Always keep session tokens confidential and use httpOnly cookies.'
              })
            }
          }
  ...
  return (
    <nav className="bg-white shadow-md sticky top-0 z-40">
      ...
      <Toast 
        ...
        show={toast.show} 
        onClose={() => setToast({ show: false, message: '', explanation: '' })} 
      />
    </nav>
  )
}
