...
declare global {
  interface Window {
    showChallengeToast?: (message: string) => void
  }
}
...
export default function CredentialsModal({ isOpen, onClose, onCorrect }: CredentialsModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isCorrect) {
      ...
      const completed = await completeChallenge(userId, 'exposeSensitiveData')
      if (completed && window.showChallengeToast) {
        window.showChallengeToast('Expose Sensitive Data challenge completed!')
      }
