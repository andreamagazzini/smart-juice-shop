  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isCorrect) {
      // Mark challenge as complete
        const completed = await completeChallenge('exposeSensitiveData')
        if (completed && window.showChallengeToast) {
          window.showChallengeToast(
            'View Other Users\' Data',
            'You accessed sensitive user data by finding database credentials exposed in the admin panel. Sensitive data like credentials should never be stored in plain text or exposed through the UI. Use secure vaults, environment variables, and proper access controls.'
          )
        }
