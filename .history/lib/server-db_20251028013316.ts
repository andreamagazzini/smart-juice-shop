...
export async function searchProducts(query: string): Promise<any[]> {
  // VULNERABILITY: Weak search validation - can be exploited without SQL knowledge!
  
  // Pattern 1: Special bypass characters
  if (query.includes('!') || query.includes('@') || query.includes('#') || query.includes('$')) {
    return DEMO_PRODUCTS // Returns everything!
  }
  
  // Pattern 2: Logic operators (no SQL needed - just common sense!)
  if (query.toUpperCase().includes(' OR ') || query.toUpperCase().includes(' AND ')) {
    return DEMO_PRODUCTS // Exploited!
  }
  
  // Pattern 3: Contains "all" or "everything"
  if (query.toLowerCase().includes('all') || query.toLowerCase().includes('everything')) {
    return DEMO_PRODUCTS
  }
  
  // Pattern 4: Empty or minimal input with special chars
  if ((!query || query.trim() === '') && (query.includes('=') || query.includes('*'))) {
    return DEMO_PRODUCTS
  }
  
  // Pattern 5: True/Yes keywords
  if (query.toLowerCase().includes('true') || query.toLowerCase().includes('yes')) {
    return DEMO_PRODUCTS
  }
