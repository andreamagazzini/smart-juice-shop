...
async function seedDatabase(db: IDBPDatabase<JuiceShopDBSchema>) {
  ...
  // Challenges
  const challenges = [
    { key: 'loginAdmin', name: 'Login as Admin', category: 'Broken Authentication', difficulty: 1, description: 'Log in with the admin user account', solved: false },
    { key: 'sessionHijacking', name: 'Session Hijacking', category: 'Broken Authentication', difficulty: 2, description: 'Hijack another user\'s session by using their session token', solved: false },
    { key: 'sqlInjectionLogin', name: 'SQL Injection (Login)', category: 'Injection', difficulty: 3, description: 'Log in with a SQL injection attack', solved: false },
    { key: 'sqlInjectionSearch', name: 'SQL Injection (Search)', category: 'Injection', difficulty: 3, description: 'Extract sensitive data via SQL injection in the search', solved: false },
    { key: 'reflectedXss', name: 'Reflected XSS', category: 'XSS', difficulty: 2, description: 'Perform a reflected XSS attack', solved: false },
    { key: 'persistedXssFeedback', name: 'Persisted XSS (Feedback)', category: 'XSS', difficulty: 4, description: 'Perform a persisted XSS attack', solved: false },
    { key: 'accessAdminSection', name: 'Access Admin Section', category: 'Broken Access Control', difficulty: 2, description: 'Access the administration section', solved: false },
    { key: 'exposeSensitiveData', name: 'Expose Sensitive Data', category: 'Broken Access Control', difficulty: 2, description: 'Access sensitive user data by entering database credentials', solved: false },
    { key: 'negativeOrder', name: 'Negative Order', category: 'Improper Input Validation', difficulty: 3, description: 'Place an order with a negative total amount', solved: false },
    { key: 'bypassPaywall', name: 'Bypass Deluxe Membership', category: 'Broken Access Control', difficulty: 4, description: 'Access a deluxe member feature without being a deluxe member', solved: false }
  ]
...
export async function resetDatabase() {
  ...
  // Clear completed challenges (but keep the challenge list itself)
  const completedChallenges = await db.getAll('completedChallenges')
  for (const challenge of completedChallenges) {
    await db.delete('completedChallenges', challenge.key)
  }
...
export async function completeChallenge(userId: number, challengeKey: string) {
  const db = await getDB()
  
  // Check if already completed
  const existing = await getCompletedChallenges(userId)
  const alreadyCompleted = existing.some(c => c.challengeId === challengeKey)
